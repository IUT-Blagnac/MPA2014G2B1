...
public class PanelEtudiant extends JPanel {
	...
	private AbstractTableModelEtudiant modele;
	private JTableEtudiant tableau;
	 ...
	 * @throws IOException
	 */
	public PanelEtudiant(File fileSelected, JFramePerso pmainFrame) throws IOException {
		super(new BorderLayout());
		mainFrame = pmainFrame;
		modele = new AbstractTableModelEtudiant(fileSelected,mainFrame);
		tableau = new JTableEtudiant(modele);
	...
	private class AddAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String groupe = JOptionPane.showInputDialog(getThis(),
					"Entrez le groupe de l'étudiant", "Groupe",
					JOptionPane.QUESTION_MESSAGE);
			if (!groupe.equals("")) { // Si la fenêtre se ferme et qu'il n'y a
										// rien, on suppose qu'il veut annuler
				String nom = JOptionPane.showInputDialog(getThis(),
						"Entrez le nom de l'étudiant", "Nom : ",
						JOptionPane.QUESTION_MESSAGE);
				String prenom = JOptionPane.showInputDialog(getThis(),
						"Entrez le prénom de l'étudiant", "Prénom : ",
						JOptionPane.QUESTION_MESSAGE);

				modele.addEtudiant(new Etudiant(nom, prenom,mainFrame.getBaseDonnees()),groupe);
			}
		}
	}
	...
	private class ModifyAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (selection.length == 0) {
				...
				JOptionPane.showMessageDialog(getThis(),
						"Veuillez sélectionner un seul étudiant");
			} else {
				String groupe = JOptionPane.showInputDialog(getThis(),
						"Entrez le groupe de l'étudiant", "Groupe : ",
						JOptionPane.QUESTION_MESSAGE);
				if (!groupe.equals("")) { // Si la fenêtre se ferme et qu'il n'y
					...
					String prenom = JOptionPane.showInputDialog(getThis(),
							...
							JOptionPane.QUESTION_MESSAGE);
					modele.addEtudiant(new Etudiant( nom, prenom,mainFrame.getBaseDonnees()),groupe);
					modele.removeEtudiant(selection[0]);
				}
